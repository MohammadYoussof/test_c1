 ...
 *
 *    $Id: swap.c 6803 2016-09-09 06:06:39Z et $
 *
 *    Revision history:
 *     $Log$
...
/* Do byte swapping on the given 4-byte unsigned integer. */
/* (Same as signed integer).                              */
void SwapUint32( uint32_t *data )
{
  SwapInt32( (int32_t *)data );
}
...
static int WaveMsgVersionMakeLocal( TRACE2X_HEADER* wvmsg, char version )
{
  ...
  if (wvmsg->nsamp > 4032/dataSize) {
     printf("WaveMsg2MakeLocal: packet from %s.%s.%s.%s has bad number of samples=%d datatype=%s\n",
 ...
 ***********************************************************************/
   ...
   {
      printf("WaveMsg2MakeLocal: packet from %s.%s.%s.%s has inconsistent "
                "header values!\n", wvmsg->sta, wvmsg->chan, wvmsg->net, wvmsg->loc );
      printf("WaveMsg2MakeLocal: header.starttime  : %.4lf\n", starttime  );
      printf("WaveMsg2MakeLocal: header.samplerate : %.1lf\n", samprate   );
      printf("WaveMsg2MakeLocal: header.nsample    : %d\n",    nsamp      );
      printf("WaveMsg2MakeLocal: header.endtime    : %.4lf\n", endtime    );
      printf("WaveMsg2MakeLocal: computed.endtime  : %.4lf\n", tShouldEnd );
      printf("WaveMsg2MakeLocal: header.endtime is not within 5 sample intervals "
                "of computed.endtime!\n" );
      return(-2);
   }
